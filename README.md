...
| **Row 4** | **Output Codec**<br>出力コーデック | `HEVC (H.265) ★推奨`（同画質で約半分のファイルサイズ、iPhone 4Kネイティブ）または `H.264`（古い再生機器との互換性重視）。 |
| | **CRF Quality**<br>画質設定 | 映像品質（0〜51、既定: `18` ★推奨）。数値が小さいほど高画質。16〜18でほぼ視覚的ロスレスを維持します。 |
