...
- **丸型（楕円）モザイク／ぼかし**: 顔の輪郭に合わせた自然な丸型形状（デフォルト）と四角形モザイクを切り替え可能。アンチエイリアス境界処理により、背景に余計なモザイクを被せず綺麗に仕上がります。
...
### 6.2 Apple Silicon (M1/M2/M3/M4) での最適化
...
### 6.4 Pass 1（顔検出）の動画デコード超高速化（全環境共通）
...
### 6.5 Pass 2（モザイク描画・レンダリング）の 3ステージ非同期パイプライン高速化（全環境共通）
...
### 6.6 色再現性（カラーマネジメント）の完全保証：色味変更の完全防止
